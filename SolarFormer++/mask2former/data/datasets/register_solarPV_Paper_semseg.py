...
def register_idrid_semseg(root, split='train'):
    meta = {
        ...
        "thing_classes": ["BG","SP","PR"],
        "stuff_classes": ["BG","SP","PR"],
        "thing_colors": [(0,0,0), (0,255,255), (255,255,255)],
        "stuff_colors": [(0,0,0), (0,255,255), (255,255,255)]
    }
    root = os.path.join(root, 'solarPV/Paper')
    image_dir = os.path.join(root, '{}_imgs'.format(split))
    gt_dir = os.path.join(root, '{}_gt'.format(split))

    sem_key = 'solarPV_paper_semseg_{}'.format(split)
