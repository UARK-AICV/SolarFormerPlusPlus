...
def register_idrid_semseg(root, split='train'):
    meta = {
        "thing_classes": ["0","1","2"],
        "stuff_classes": ["BG","SP","Branch"],
        "thing_colors": [(0,0,0), (0,255,0), (255,0,0)],
        "stuff_colors": [(0,0,0),(0,255,0), (255,0,0)],
    }
    root = os.path.join(root, 'solarPV/Branch')
    image_dir = os.path.join(root, '{}_imgs'.format(split))
    gt_dir = os.path.join(root, '{}_gt'.format(split))

    sem_key = 'solarPV_branch_semseg_{}'.format(split)
