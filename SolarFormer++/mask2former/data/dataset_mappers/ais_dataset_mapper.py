...
from .coco_instance_new_baseline_dataset_mapper import convert_coco_poly_to_mask
...
class AmodalDatasetMapper:
    ...
    def _amodal_annotations_to_instances(self, annos, image_size, mask_format="polygon"):
        ...
        if len(annos) and "amodal_segm" in annos[0]:
            ...
            target.gt_masks = self._segms_to_polygon_masks(amodal_segms, mask_format, image_size)
    ...
    def _transform_annotations(self, dataset_dict, transforms, image_shape):
        ...
        if hasattr(instances, 'gt_masks'):
            if hasattr(instances.gt_masks, 'polygons'):
                instances.gt_amodal_masks = convert_coco_poly_to_mask(instances.gt_amodal_masks.polygons, h, w)
                instances.gt_visible_masks = convert_coco_poly_to_mask(instances.gt_visible_masks.polygons, h, w)
                instances.gt_background_objs_masks = convert_coco_poly_to_mask(instances.gt_background_objs_masks.polygons, h, w)
                instances.gt_occluder_masks = convert_coco_poly_to_mask(instances.gt_occluder_masks.polygons, h, w)
                instances.gt_masks = convert_coco_poly_to_mask(instances.gt_masks.polygons, h, w)
            else:
                instances.gt_amodal_masks = instances.gt_amodal_masks.tensor
                instances.gt_visible_masks = instances.gt_visible_masks.tensor
                instances.gt_background_objs_masks = instances.gt_background_objs_masks.tensor
                instances.gt_occluder_masks = instances.gt_occluder_masks.tensor
